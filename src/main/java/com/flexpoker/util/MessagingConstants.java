package com.flexpoker.util;

public class MessagingConstants {

    public static final String CHAT_GLOBAL_USER = "/topic/chat/global/user";

    public static final String CHAT_GLOBAL_SYSTEM = "/topic/chat/global/system";
    
    public static final String CHAT_GAME_USER = "/topic/chat/game/user/%s";
    
    public static final String CHAT_GAME_SYSTEM = "/topic/chat/game/system/%s";
    
    public static final String CHAT_TABLE_USER = "/topic/chat/table/user/%s";
    
    public static final String CHAT_TABLE_SYSTEM = "/topic/chat/table/system/%s";
    
    public static final String CHAT_PERSONAL_USER = "/topic/chat/personal/user/%s";
    
    public static final String CHAT_PERSONAL_SYSTEM = "/topic/chat/personal/system/%s";

}
