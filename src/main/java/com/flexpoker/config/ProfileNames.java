package com.flexpoker.config;

public class ProfileNames {

    /** In-memory and default */
    public static final String DEFAULT = "default";
    public static final String LOGIN_INMEMORY = "login-inmemory";
    public static final String SIGNUP_INMEMORY = "signup-inmemory";
    public static final String GAME_COMMAND_INMEMORY = "game-command-inmemory";
    public static final String GAME_QUERY_INMEMORY = "game-query-inmemory";
    public static final String TABLE_COMMAND_INMEMORY = "table-command-inmemory";
    public static final String TABLE_QUERY_INMEMORY = "table-query-inmemory";

    /** Redis */
    public static final String REDIS = "redis";
    public static final String LOGIN_REDIS = "login-redis";
    public static final String SIGNUP_REDIS = "signup-redis";
    public static final String GAME_COMMAND_REDIS = "game-command-redis";
    public static final String GAME_QUERY_REDIS = "game-query-redis";
    public static final String TABLE_COMMAND_REDIS = "table-command-redis";
    public static final String TABLE_QUERY_REDIS = "table-query-redis";

}
