...
class PotHandler(
    private val gameId: UUID, private val tableId: UUID, private val handId: UUID,
    private val handEvaluationList: List<HandEvaluation>
) {
    private val pots: MutableSet<Pot> = HashSet()

    fun removePlayerFromAllPots(playerId: UUID) {
        pots.forEach { it.removePlayer(playerId) }
    }

    fun addToPot(potId: UUID, amountToAdd: Int) {
        pots.first { it.id == potId }.addChips(amountToAdd)
    }

    fun closePot(potId: UUID) {
        pots.first { it.id == potId }.closePot()
    }

    fun addNewPot(potId: UUID, playersInvolved: Set<UUID?>) {
    ...
    fun fetchPlayersRequiredToShowCards(playersStillInHand: Set<UUID>): Set<UUID> {
    ...
    fun fetchChipsWon(playersStillInHand: Set<UUID>): PMap<UUID, Int> {
     ...
     */
    fun calculatePots(chipsInFrontMap: Map<UUID, Int>, chipsInBackMap: Map<UUID, Int>): List<TableEvent> {
        ...
        for (chipsPerLevel in distinctChipsInFrontAmounts) {
            ...
            if (openPotOptional == null) {
                ...
                addNewPot(potCreatedEvent.potId, potCreatedEvent.playersInvolved)
            }
            ...
            addToPot(potAmountIncreasedEvent.potId, potAmountIncreasedEvent.amountIncreased)
